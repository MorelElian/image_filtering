...
main( int argc, char ** argv )
{
    ...
    {
        int data_to_receive;
        data_to_receive = chunk_size + (i < remainder ? 1 : 0);
        int j;
        for(j =0;j<data_to_receive;j++)
        {
            int i_image;
            i_image = i * chunk_size + (i < remainder ? i : remainder) +j;
            pixel* tmp = malloc(image->width[i_image] * image->height[i_image] * sizeof(pixel));
            
            MPI_Recv(tmp,image->width[i_image] * image->height[i_image],MPI_PIXEL,i,0,MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            image->p[i_image] = tmp;
        }
